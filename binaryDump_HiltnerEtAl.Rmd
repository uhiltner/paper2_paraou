---
title: Binary dump for the Appendix A3 of the paper Hiltner et al.
...
# enter file name endings of simulation result data to be validated
...
filterSc_prod <- "prod$" # gpp,npp, resp,mn
...
tidy_logResults <- function(df, logEvent, nolpy, undistGrowthSc){
...
filesSim <- dir(path = dataPathSim, pattern = "_paracouForest_")
...
simData_agb <- dataSim %>% 
  ...
  tidy_logResults(logEvent = logEvent, nolpy = 1, undistGrowthSc = paste0(undistGrowthSc, "bt_th")) #filterSc_agb
# basal area
simData_ba <- dataSim %>% 
  filter(str_detect(filename, filterSc_ba)) %>%
  select(filename:Time , TotalBasalArea:BasalAreaPerPFT_8,DBH_cutth, Treatment) %>%
  # remove duplicate second year in results of logging scenarios
  tidy_logResults(logEvent = logEvent, nolpy = 1, undistGrowthSc = paste0(undistGrowthSc,"ba_th"))
# stem numbers
simData_sn <- dataSim %>% 
  filter(str_detect(filename, filterSc_sn)) %>%
  select(filename:Time , TotalNumber:NumberPerPFT_8,DBH_cutth, Treatment) %>%
  # remove duplicate second year in results of logging scenarios
  tidy_logResults(logEvent = logEvent, nolpy = 1, undistGrowthSc = paste0(undistGrowthSc,"n_th"))
...
simData_ha <- dataSim %>% 
  ...
  # remove duplicate second year in results of logging scenarios
  tidy_logResults(logEvent = logEvent, nolpy = 16, undistGrowthSc = paste0(undistGrowthSc,"ha"))
...
simData_prod <- dataSim %>% 
  ...
  # remove duplicate second year in results of logging scenarios
  tidy_logResults(logEvent = logEvent, nolpy = 1, undistGrowthSc = paste0(undistGrowthSc,"prod"))
# leaf area index
simData_lai <- dataSim %>% 
  filter(str_detect(filename, filterSc_lai)) %>%
  select(filename, Time, LAI,DBH_cutth, Treatment) %>%
  # remove duplicate second year in results of logging scenarios
  tidy_logResults(logEvent = logEvent, nolpy = 1, undistGrowthSc = paste0(undistGrowthSc,"lai_mean"))
# bole volume
simData_bv <- dataSim %>% 
  filter(str_detect(filename, filterSc_bv)) %>%
  select(filename, Time, TotalBoleVolume:BoleVolumePerPFT_8,DBH_cutth, Treatment) %>%
  # remove duplicate second year in results of logging scenarios
  tidy_logResults(logEvent = logEvent, nolpy = 1, undistGrowthSc = paste0(undistGrowthSc,"bv_th"))
...
if(saveFig) ggsave("calibRes_agb.jpg", width = 4.0, height = 4.0)
...
if(saveFig) ggsave("calibRes_mnBa.jpg", width = 4, height = 4)
...
if(saveFig) ggsave("calibRes_agbMnBa.jpg", width = 8, height = 3.5)

rm(fig31a, fig31b)

```
@fig Fig 3.1: Calibration results. Comparison between the observed (dots) and simulated (line gra
...
if(saveFig) ggsave("validRes_agbT1.jpg", width = 4, height = 4)   
...
if(saveFig) ggsave("parRes_dam1.jpg", width = 4, height = 4)
...
if(saveFig) ggsave("validRes_agbDam1.jpg", width = 8, height = 3.5)
